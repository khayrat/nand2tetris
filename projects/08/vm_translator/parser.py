...
class Parser:
    def __init__(self, fn=None, stream=None):
        ...
        self.c_push_pop = re.compile('^(push|pop)\s+(\w+)\s+(\d+)(\s*//.*)?$')    
        # program flow commands
        self.c_lable_goto = re.compile('^(label|goto)\s+(' + _symbol +')(\s*//.*)?$')
        self.c_if_goto = re.compile('^(if-goto)\s+(' + _symbol +')(\s*//.*)?$')
        # function calling commands
        self.c_return = re.compile('^return$')
        self.c_fun_call = re.compile('^(call|function)\s+('+_symbol+')(\s+(\d+(\s+\d+)*))?(\s*//.*)?$')
        # arithmetic logical commands
        self.c_arithmetic = re.compile('^(\w+)(\s*//.*)?$')    
        # yieldings
        self.cmdType = None
        self._arg1 = None
        self._arg2 = None
    ...
    def _decompose(self, cmd):
        ...
        print("parsing error. could not parse command '%s'" % cmd) 
        sys.exit(1)
...
if __name__ == '__main__':
    ...
    while p.hasMoreCommands():
        ...
        a1=p.arg1()
        a2=p.arg2()
        print("type: %s, arg1: %s, arg2: %s " % (t, a1, a2))
